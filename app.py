...
"""
from util.pages.bdpage import building_page
...
from util.pages.hmpage import home_page
...
class MultiApp:
    ...
    def run(self):
        ...
        app = st.sidebar.selectbox(
            "选择页面", self.apps, format_func=lambda app: app["title"]
        )
        st.sidebar.markdown("---")
        app["function"]()
...
app.add_app("首页", home_page)
#app.add_app("团队成员", member_page)
# app.add_app("System Overview", overview_page)
app.add_app("建筑数据", building_page)
app.add_app("结果可视化", data_page)
